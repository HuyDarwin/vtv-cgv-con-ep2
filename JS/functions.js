...
$(function () {
	...
	(function (con) {
		...
		con.CongTuNguoiChoi = function(num,is_checking_player_sn_answers){
			if (!is_checking_player_sn_answers){
				player_accepted_words += num;
				update(ref(db), { player_accepted_words : player_accepted_words })
			}
			else{
				player_sn_accepted_words += num;
				update(ref(db), { player_sn_accepted_words : player_sn_accepted_words })
			}
		}
		con.ResetSoTu = function(){
			player_accepted_words = 0;
			player_sn_accepted_words = 0;
			update(ref(db), {
				player_accepted_words : player_accepted_words,
				player_sn_accepted_words : player_sn_accepted_words
			})
		}
